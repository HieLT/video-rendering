import asyncio,json,re,time,traceback,sys,sqlite3
...
OUT=Path('diagnostics/frame_live_test_'+str(int(time.time())))
...
def clean(v):
 ...
 return v
for name,bg,rect in [('A',(245,225,185),(40,180,160,300)),('B',(180,225,245),(352,180,472,300))]:
 im=Image.new('RGB',(512,512),bg);d=ImageDraw.Draw(im);d.rectangle(rect,fill=(210,30,45));d.line((0,400,512,400),fill=(30,40,50),width=8);im.save(OUT/(name+'.png'))
PROMPT='A locked camera shows a red square on a flat background, above a dark horizontal line. The square moves smoothly across the scene. No text, no cuts, no audio.'
async def main():
 ...
 async with async_playwright() as p:
  ...
  async def response_log(response):
   ...
   try:
    ...
    print('RESPONSE',response.status,'captcha' if captcha_rejected else data[:700],flush=True)
  ...
  async def intercept(route):
   ...
   if params.get('duration')!=5 or len(found)!=2:
  ...
  try:
   for variant in ([sys.argv[2]] if len(sys.argv)>2 else ['baseline','roles']):
    ...
    for name in (['B','A'] if variant=='reverse' else ['A','B']):
    ...
    await box.fill(PROMPT);await page.screenshot(path=str(OUT/(variant+'_before.png')))
